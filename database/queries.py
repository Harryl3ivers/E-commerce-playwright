...
def get_product_by_name(product_name,connection):
    cursor = connection.cursor()
    cursor.execute("SELECT * FROM products WHERE NAME = ?",(product_name,))
    product = cursor.fetchone()
    return product

def get_product_by_price(product_name,connection):
    cursor = connection.cursor()
...
def get_order_product(order_id,connection):
